...
/**
 * TraceValidityAction activates ConstraintLocator.debug() for an unambiguous validatable-node + constraining-node selection.
 */
public final class TraceValidityAction extends Action implements ISelectionChangedListener
{
	protected final @NonNull ValidityView validityView;
	protected final @NonNull ISelectionProvider selectionProvider;

	public TraceValidityAction(@NonNull ValidityView validityView, @NonNull ISelectionProvider selectionProvider) {
		super(ValidityUIMessages.ValidityView_Action_TraceValidity_Title);
		this.validityView = validityView;
		this.selectionProvider = selectionProvider;
		selectionProvider.addSelectionChangedListener(this);
		setToolTipText(ValidityUIMessages.ValidityView_Action_TraceValidity_ToolTipText);
		URL image = (URL) ValidityUIPlugin.INSTANCE.getImage(ValidityUIMessages.ValidityView_Action_TraceValidity_ImageLocation);
		...
		if (selection instanceof StructuredSelection) {
			...
			//	final @Nullable Shell shell = validityView.getViewSite().getShell();
				...
				//	final ResultConstrainingNode finalResultConstrainingNode = resultConstrainingNode;
					...
					AbstractEnvironmentFactory environmentFactory = (AbstractEnvironmentFactory)validityManager.getEnvironmentFactory();
					boolean savedIsEvaluationTracingEnabled = environmentFactory.isEvaluationTracingEnabled();
					try {
						environmentFactory.setEvaluationTracingEnabled(true);
						//	Monitor monitor = new NullProgressMonitor();
						Result result = resultConstrainingNode.getWorstResult();
						assert result != null;
						constraintLocator.validate(result, validityManager, null);					
					}
					finally {
						environmentFactory.setEvaluationTracingEnabled(savedIsEvaluationTracingEnabled);
					}
